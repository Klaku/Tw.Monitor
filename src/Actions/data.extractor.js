...
module.exports = async (name, maper, filter) => {
    ...
    console.time(name);
    try {
        let response = await fetch(`https://${config.Minner.World}.${config.Minner.Domain}/map/${name}.txt`);
        ...
        if (collection.length > 0) {
            ...
            let result = await client.db(config.DB.Name).collection(name).insertMany(collection);
