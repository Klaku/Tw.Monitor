...
module.exports = async () => {
    try {
        ...
        allyList = allyList.filter((item, index, array) => {
            return item.rank <= 40 && array.map((x) => x.id).indexOf(item.id) == index;
        });
            ...
            try {
                ...
                for (let playerIndex = 0; playerIndex < players.length; playerIndex++) {
                    ...
                    try {
                        ...
                        let Filter24h = (item) => {
                            return item.created > new Date(Date.now() - 1000 * 60 * 60 * 24);
                        };
                        ...
                        playersCollection.push({
                            ...
                            villages: {
                                now: player.villages,
                                h24: playerData.filter(Filter24h).sort(SortDescending)[0].villages,
                                d7: playerData.sort(SortAscending)[0].villages,
                            },
                            RA: {
                                now: isEmpty(playerRA) ? 0 : playerRA.sort(SortDescending)[0].score,
                                h24: isEmpty(playerRA) ? 0 : playerRA.filter(Filter24h).sort(SortDescending)[0].score,
                                d7: isEmpty(playerRA) ? 0 : playerRA.sort(SortAscending)[0].score,
                            },
                            RAA: {
                                now: isEmpty(playerRAA) ? 0 : playerRAA.sort(SortDescending)[0].score,
                                h24: isEmpty(playerRAA) ? 0 : playerRAA.filter(Filter24h).sort(SortDescending)[0].score,
                                d7: isEmpty(playerRAA) ? 0 : playerRAA.sort(SortAscending)[0].score,
                            },
                            RD: {
                                now: isEmpty(playerRD) ? 0 : playerRD.sort(SortDescending)[0].score,
                                h24: isEmpty(playerRD) ? 0 : playerRD.filter(Filter24h).sort(SortDescending)[0].score,
