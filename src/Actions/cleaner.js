...
module.exports = async () => {
    try {
        ...
        let result = await collections.ally.deleteMany({ "created": { $le: new Date(Date.now() - (1000 * 60 * 60 * 24 * 14)) } });
        console.log(`Deleted ${result.deletedCount} allies`);
        await collections.player.deleteMany({ "created": { $le: new Date(Date.now() - (1000 * 60 * 60 * 24 * 14)) } });
        console.log(`Deleted ${result.deletedCount} players`);
        await collections.RAA.deleteMany({ "created": { $le: new Date(Date.now() - (1000 * 60 * 60 * 24 * 14)) } });
        console.log(`Deleted ${result.deletedCount} RAA`);
        await collections.RA.deleteMany({ "created": { $le: new Date(Date.now() - (1000 * 60 * 60 * 24 * 14)) } });
        console.log(`Deleted ${result.deletedCount} RA`);
        await collections.RD.deleteMany({ "created": { $le: new Date(Date.now() - (1000 * 60 * 60 * 24 * 14)) } });
